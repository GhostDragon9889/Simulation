...
- Throughout the repository, the name `swarm_robotics` only serves as an example and we provide a script to rename all the references to it automatically:

```bash
# Enter the repository
cd IsaacLabExtensionTemplate
# Rename all occurrences of swarm_robotics (in files/directories) to your_fancy_extension_name
...
```bash
python -m pip install -e source/swarm_robotics
```
...
We provide an example UI extension that will load upon enabling your extension defined in `source/swarm_robotics/swarm_robotics/ui_extension_example.py`.
...
```json
{
    "python.analysis.extraPaths": [
        "<path-to-ext-repo>/source/swarm_robotics"
    ]
}
```
